...
任一结点的左子树和右子树的高度相差不超过1

TBhan

Crilly

右旋:左边的孩子往上提,父亲变成右孩子.如果左边的孩子有右孩子,就变成父亲的左孩子
...
private:
	...
	size_t GetHeight(PNODE pNode);// 获取指定结点的高度,空为0
...
void CTree::AdjustHeight(PNODE pNewNode)
{
    // 从父节点到根结点,一路调整高度
    PNODE pFather = pNewNode->m_pFather;
    PNODE pChild = pNewNode;
    while (pFather != nullptr)
    {
        if (pFather->m_nHeight == pChild->m_nHeight)
        {
            pFather->m_nHeight++; 
            pChild = pFather;// 只是函数内部变量,不会影响外部二叉树的除高度以外的参数
...
size_t CTree::GetHeight(PNODE pNode)
{
    if (pNode == nullptr)
    {
        return 0;
...
![1](https://alist.hmbb313.top/d/Baidunetdisk/Images/Cracker/40/403DataStruct/10/1.png)

![2](https://alist.hmbb313.top/d/Baidunetdisk/Images/Cracker/40/403DataStruct/10/2.png)